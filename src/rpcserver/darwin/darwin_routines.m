...
routine_status_t routine_get_class_list(const ProtobufCMessage *in_msg, ProtobufCMessage **out_msg) {
  ...
  Rpc__ReplyGetClassList *reply_get_class_list = NULL;
  Class *class_list = NULL;
  int count = 0;

  reply_get_class_list = (Rpc__ReplyGetClassList *) malloc(sizeof *reply_get_class_list);
  CHECK(reply_get_class_list != NULL);
  rpc__reply_get_class_list__init(reply_get_class_list);
  ...
  reply_get_class_list->classes = (Rpc__ObjcClass **) calloc((size_t) count, sizeof(Rpc__ObjcClass *));
  ...
  for (int i = 0; i < count; ++i) {
    ...
    Rpc__ObjcClass *cls = (Rpc__ObjcClass *) malloc(sizeof *cls);
    CHECK(cls != NULL);
    rpc__objc_class__init(cls);
...
routine_status_t routine_show_class(const ProtobufCMessage *in_msg, ProtobufCMessage **out_msg) {
  ...
  const Rpc__RequestShowClass *request_show_class = (const Rpc__RequestShowClass *) in_msg;
  Rpc__ReplyShowClass *reply_show_class = NULL;

  reply_show_class = (Rpc__ReplyShowClass *) malloc(sizeof *reply_show_class);
  CHECK(reply_show_class != NULL);
  rpc__reply_show_class__init(reply_show_class);
...
routine_status_t routine_show_object(const ProtobufCMessage *in_msg, ProtobufCMessage **out_msg) {
  ...
  const Rpc__RequestShowObject *request_show_object = (const Rpc__RequestShowObject *) in_msg;
  Rpc__ReplyShowObject *reply_show_object = NULL;

  reply_show_object = (Rpc__ReplyShowObject *) malloc(sizeof *reply_show_object);
  CHECK(reply_show_object != NULL);
  rpc__reply_show_object__init(reply_show_object);
...
routine_status_t routine_get_dummy_block(const ProtobufCMessage *in_msg, ProtobufCMessage **out_msg) {
    Rpc__ReplyDummyBlock *reply_dummy_block = malloc(sizeof *reply_dummy_block);
    CHECK(reply_dummy_block != NULL);
    rpc__reply_dummy_block__init(reply_dummy_block);
...
void cleanup_get_class_list(ProtobufCMessage *reply) {
    Rpc__ReplyGetClassList *r = (Rpc__ReplyGetClassList *) reply;
    if (!r || !r->classes) return;
    for (size_t i = 0; i < r->n_classes; ++i) {
        Rpc__ObjcClass *cls = r->classes[i];
        if (!cls) continue;
        if (cls->name) {
            free(cls->name);
            cls->name = NULL;
...
void cleanup_show_class(ProtobufCMessage *reply) {
    Rpc__ReplyShowClass *r = (Rpc__ReplyShowClass *) reply;
    if (r && r->description) {
        free(r->description);
        r->description = NULL;
    }
...
void cleanup_show_object(ProtobufCMessage *reply) {
    Rpc__ReplyShowObject *r = (Rpc__ReplyShowObject *) reply;
    if (r && r->description) {
        free(r->description);
        r->description = NULL;
    }
