...
void call_function(intptr_t address, size_t va_list_index, size_t argc, Rpc__Argument **p_argv, Rpc__ReplyCall *resp) {
    ...
    for (idx_argv = 0; idx_argv < argc; idx_argv++) {
        ...
        // `v_bytes.data`.
        current_arg =
            (p_argv[idx_argv]->type_case == RPC__ARGUMENT__TYPE_V_BYTES ? (intptr_t *) &p_argv[idx_argv]->v_bytes.data
...
void call_function(intptr_t address, size_t va_list_index, size_t argc, Rpc__Argument **p_argv,
                   Rpc__ReplyCall *response) {
    s64 return_val;
    TRACE("enter");
    ...
    for (size_t i = 0; i < argc; i++) {
        ...
        case RPC__ARGUMENT__TYPE_V_DOUBLE: args[i] = p_argv[i]->v_double; break;
        case RPC__ARGUMENT__TYPE_V_INT: args[i] = p_argv[i]->v_int; break;
        case RPC__ARGUMENT__TYPE_V_STR: args[i] = (uint64_t) p_argv[i]->v_str; break;
        case RPC__ARGUMENT__TYPE_V_BYTES: args[i] = (uint64_t) p_argv[i]->v_bytes.data; break;
        default: break;
        }
    }
    ...
    response->return_values_case = RPC__REPLY_CALL__RETURN_VALUES_RETURN_VALUE;
