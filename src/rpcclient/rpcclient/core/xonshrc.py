...
def _pretty_json(buf, colored=True, default=_default_json_encoder):
    ...
    if colored:
        colorful_json = highlight(
            formatted_json, lexers.JsonLexer(), formatters.TerminalTrueColorFormatter(style="stata-dark")
        )
...
class XonshRc:
    ...
    def _rpc_press_keys(
        ...
        key: Annotated[
            ...
            Arg(
                nargs="+",
                completer=lambda xsh, action, completer, alias, command: ["power", "home", "volup", "voldown", "mute"],
            ),
        ],
    ):
        """
    ...
    def _rpc_ln(
        self,
        src: Annotated[str, Arg(completer=path_completer)],
    ...
    def _rpc_mkdir(
        self,
        filename: Annotated[str, Arg(completer=path_completer)],
        parents: bool = False,
    ...
    def _rpc_pull(
        self,
        files: Annotated[list[str], Arg(nargs="+", completer=path_completer)],
...
# actual RC contents
XSH.aliases["xontrib"](["load", "z", "argcomplete", "coreutils", "fzf-widgets", "jedi"])
