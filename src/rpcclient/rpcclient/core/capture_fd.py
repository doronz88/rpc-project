...
class CaptureFD:
    ...
    def __enter__(self) -> "CaptureFD":
        with self._client.safe_malloc(FD_SIZE * 2) as socket_pair:
            ...
            if self._client.symbols.socketpair(AF_UNIX, SOCK_STREAM, 0, socket_pair) != 0:
        ...
        if self._client.symbols.dup2(self._socket_pair[0], self.fd) < 0:
    ...
    def __exit__(self, _type, value, traceback) -> None:
        if self._backupfd is not None:
            if self._client.symbols.dup2(self._backupfd, self.fd) < 0:
                self._client.raise_errno_exception("dup2 backup-fd failed")
            if self._client.symbols.close(self._backupfd) != 0:
                self._client.raise_errno_exception("close backupfd failed")
            self._backupfd = None
        if self._socket_pair is not None:
            if self._client.symbols.close(self._socket_pair[0]) != 0:
                self._client.raise_errno_exception(f"close _socket_pair[0] {self._socket_pair[0]} failed")
            if self._client.symbols.close(self._socket_pair[1]) != 0:
                self._client.raise_errno_exception(f"close _socket_pair[1] {self._socket_pair[1]} failed")
            ...
            with self._client.safe_malloc(READ_SIZE) as buff:
                ...
                while read == READ_SIZE:
                    with self._client.safe_malloc(pollfd.sizeof()) as pfds:
                        ...
                        if self._client.symbols.poll(pfds, 1, 0) != 1:
                            return data
                    read = self._client.symbols.read(self._socket_pair[1], buff, READ_SIZE).c_int32
                    if read == -1:
