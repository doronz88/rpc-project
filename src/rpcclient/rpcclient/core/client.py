...
class CoreClient:
    ...
    DEFAULT_ARGV: typing.ClassVar = ["/bin/sh"]
    DEFAULT_ENVP: typing.ClassVar = []
    ...
    def call(
        self,
        address: int,
        argv: Optional[list[int]] = None,
        return_float64=False,
        return_float32=False,
        return_raw=False,
        va_list_index: int = 0xFFFF,
    ) -> typing.Union[float, Symbol, Any]:
    ...
    def listdir(self, path: str):
        ...
        for entry in ret.dir_entries:
            ...
            entries.append(
                ProtocolDirent(
                    d_inode=entry.lstat.st_ino, d_type=entry.d_type, d_name=entry.d_name, lstat=lstat, stat=stat
                )
            )
        return entries
