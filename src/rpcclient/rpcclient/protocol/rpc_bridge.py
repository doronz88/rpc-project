...
BASIC_MESSAGES = RpcMessageRegistry(modules=['rpcclient.protos.rpc_pb2'])


class RpcBridge:
    def __init__(self, sock: socket.socket, messages: Optional[RpcMessageRegistry] = None) -> None:
        # Maps request class -> binding
        self._messages: RpcMessageRegistry = messages or BASIC_MESSAGES.clone()
        self.handshake: Union[Handshake, None] = None
    ...
    def rpc_call(self, msg_id: int, **kwargs):
        """
        Resolve msg_id/reply class from request_msg's type, build RpcMessage, sendNSS, parse reply.
        """
        req = self._messages.get(msg_id)(**kwargs)
        msg = RpcMessage(
        ...
        reply_class = self._messages.get(rep_msg.msg_id)
        if reply_class is None:
            raise ValueError(f'Unknown reply class msg_id: {rep_msg.msg_id}')
        rep = reply_class()
