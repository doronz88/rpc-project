...
class Bluetooth:
    ...
    def __init__(self, client):
        ...
        if not client.getenv(self._ENV_QUEUE_SET):
            bluetooth_manager_class.objc_call(
                "setSharedInstanceQueue:", self._client.symbols.dispatch_queue_create(0, 0)
            )
            client.setenv(self._ENV_QUEUE_SET, "1")
        self._bluetooth_manager = bluetooth_manager_class.objc_call("sharedInstance")

    def is_on(self):
        return self._bluetooth_manager.objc_call("enabled") == 1
