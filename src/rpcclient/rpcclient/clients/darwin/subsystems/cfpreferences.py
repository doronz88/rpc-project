...
class CFPreferences(ClientBound["DarwinClient[DarwinSymbolT_co]"], Generic[DarwinSymbolT_co]):
    """
    ...
    async def get_keys(
        ...
        keys = await (
            await self._client.symbols.CFPreferencesCopyKeyList(
                await self._client.cf(application_id),
                await self._client.cf(username),
                await self._client.cf(hostname),
            )
        ).py()
    ...
    async def get_value(
        ...
        value = await (
            await self._client.symbols.CFPreferencesCopyValue(
    ...
    async def get_dict(
        ...
        return {k: await self.get_value(k, application_id, username, hostname) for k in key_list}
    ...
    async def set_dict(
        ...
        with suppress(NoSuchPreferenceError):
            await self.clear(application_id, username, hostname)
        await self.update_dict(d, application_id, username, hostname)
